import torchaudio
import torch
from Encoder_Decoder_Training.Data_Processing.Chunk_Dataset import AudioChunkDataSet
import os
import constants
import pandas as pd
import logging 
...
class PreProcessor:
    ...
    """
    def __init__(self, input_audio_dir=constants.RAW_MP3_DIR,
                 output_dir= constants.ENCODER_DECODER_PROCESSED_DIR, 
                 desired_sample_rate = 48000, 
                 tensor_len =  2**18) -> None:
        self.meta_data = None
    ...
    def chunk_single_song(self, audio_file):
        ...
        """
        splitted, sample_rate, start_times, time_per_split = self.split_audio(os.path.join(self.input_dir,
                                                                                                    audio_file))
        ...
        for i in range(len(splitted)):
            ...
            torchaudio.save(music_path,splitted[i],
                            sample_rate=self.sample_rate, channels_first=True)
        return temp_df
    ...
    def preprocess(self, verbose = False)->None:
        ...
        with mp.Pool(mp.cpu_count()-2) as p:
            return_meta_list = list(tqdm(p.imap(self.chunk_single_song, audio_files_list), total=len(audio_files_list)))
    ...
    def split_audio(self, audio_file_path,verbose = False):
        ...
        if self.sample_rate!= sample_rate and verbose:
            logging.info('Sample rate not desired in file with path: ' + audio_file_path)
        splitted= torch.split(audio_tensor,split_size_or_sections=self.tensor_len,dim =1)[:-1]
        time_per_split = (audio_tensor.shape[1]/sample_rate)/len(splitted)
        start_times = (torch.arange(start = 0, end = len(splitted))*time_per_split).tolist()
    ...
    def return_dataset(self,meta_data:pd.DataFrame = None, additional_meta_data:list= None):
        """
        ...
        def _get_audio_sample_path_from_meta_data(index):
            ...
            return os.path.join(self.audio_dir,self.naming_convention(row['song_name'], int(row['start_time'])))+'.wav'
        ...
        if additional_meta_data:
            ...
            return AudioChunkDataSet(meta_data[meta_data_list],self.audio_dir,
                                     _get_audio_sample_path_from_meta_data,get_metadata=meta_data_list)
        else:
            return AudioChunkDataSet(meta_data,self.audio_dir,
    ...
    def split_into_train_val(self,train_prop = 0.95, additional_meta_data: list = None):
        ...
        return self.return_dataset(train_meta_data, additional_meta_data), self.return_dataset(val_meta_data, additional_meta_data)
